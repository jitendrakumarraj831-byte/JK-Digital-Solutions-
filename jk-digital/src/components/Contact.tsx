...
const contacts = [
  {
    ...
    g: "linear-gradient(135deg,rgba(37,211,102,0.22),rgba(18,140,126,0.15))",
    border: "rgba(52,211,153,0.3)", color: "#4ade80",
  },
  {
    ...
    label: "Call", value: "+91 85418 49118", sub: "Mon–Sat 9AM–8PM",
    href: "tel:+918541849118",
    g: "linear-gradient(135deg,rgba(59,130,246,0.22),rgba(99,102,241,0.15))",
    border: "rgba(96,165,250,0.3)", color: "#93c5fd",
  },
  {
    ...
    label: "Email", value: "jkdigitalsolutionfbg@gmail.com", sub: "Reply in 2–4h",
    href: "mailto:jkdigitalsolutionfbg@gmail.com",
    g: "linear-gradient(135deg,rgba(168,85,247,0.22),rgba(124,58,237,0.15))",
    border: "rgba(216,180,254,0.3)", color: "#c4b5fd",
  },
  {
    ...
    label: "Location", value: "Forbesganj, Araria, Bihar", sub: "854318",
    href: "https://maps.google.com/?q=Forbesganj+Bihar",
    g: "linear-gradient(135deg,rgba(245,158,11,0.2),rgba(234,88,12,0.14))",
    border: "rgba(251,191,36,0.28)", color: "#fde68a",
  },
];
...
const labelSt: React.CSSProperties = {
  ...
  color: "rgba(228,220,255,0.35)", marginBottom: "8px",
};
...
export default function Contact() {
  ...
  const sub = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else { setStatus("error"); setMsg(d.error || "कुछ गड़बड़ी हुई। दोबारा कोशिश करें।"); }
  ...
  return (
    <section id="contact" style={{
      padding: "112px 0",
      background: "linear-gradient(180deg, #0a041e 0%, #07001a 50%, #040010 100%)",
      ...
      <div className="wrap">
        <div style={{
          ...
          <div>
            ...
            <p style={{ color: "rgba(228,220,255,0.45)", fontSize: "17px", lineHeight: 1.8, marginBottom: "44px", maxWidth: "400px" }}>
              Form fill करें या directly reach करें। हम 30 minutes के अंदर respond करते हैं।
            </p>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "36px" }}>
              {contacts.map(c => (
                <a key={c.label} href={c.href} target="_blank" rel="noopener noreferrer"
                  ...
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translateX(6px)"; (e.currentTarget as HTMLElement).style.boxShadow = `0 8px 30px ${c.border}`; }}
                  ...
                  <div style={{ width: "44px", height: "44px", borderRadius: "12px", background: `${c.border.replace("0.3","0.25")}`, color: c.color, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: `1px solid ${c.border}` }}>{c.icon}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.09em", color: "rgba(228,220,255,0.35)", marginBottom: "2px" }}>{c.label}</p>
                    <p style={{ fontWeight: 600, fontSize: "14px", color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.value}</p>
                  </div>
                  <span style={{ fontSize: "11px", color: c.color, fontWeight: 600, flexShrink: 0, opacity: 0.7 }}>{c.sub}</span>
                </a>
              ))}
            </div>
            ...
            <div style={{
              ...
              <p style={{ fontWeight: 700, color: "#c4b5fd", fontSize: "12px", marginBottom: "8px" }}>🕐 Business Hours</p>
              <p style={{ fontSize: "13px", color: "rgba(228,220,255,0.45)", lineHeight: 1.85 }}>
          ...
          <div style={{
            ...
            <div style={{ marginBottom: "28px" }}>
              ...
              <p style={{ color: "rgba(228,220,255,0.4)", fontSize: "14px", lineHeight: 1.6 }}>
                हम personally analyze करके बताएंगे आपके business के लिए best strategy।
              </p>
            </div>

            <div style={{ height: "1px", background: "rgba(167,139,250,0.12)", marginBottom: "28px" }} />

            <form onSubmit={sub} style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "14px" }}>
                <div>
                  <label style={labelSt}>आपका नाम *</label>
                  <input type="text" name="name" value={form.name} onChange={chg} required placeholder="Full Name" className="inp" />
                </div>
                <div>
                  <label style={labelSt}>WhatsApp Number *</label>
                  <input type="tel" name="phone" value={form.phone} onChange={chg} required placeholder="10-digit" maxLength={10} className="inp" />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea name="message" value={form.message} onChange={chg} required rows={4}
                  placeholder="Website, SEO, Google Ads, GMB — बताएं।" className="inp" style={{ resize: "none" }} />
              </div>

              {status === "success" && (
                <div style={{ padding: "14px 18px", borderRadius: "12px", background: "rgba(16,185,129,0.1)", border: "1px solid rgba(52,211,153,0.25)" }}>
                  <p style={{ color: "#34d399", fontSize: "13px", fontWeight: 500 }}>✓ {msg}</p>
                </div>
              )}
              {status === "error" && (
                <div style={{ padding: "14px 18px", borderRadius: "12px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)" }}>
              ...
              <button type="submit" disabled={status === "loading"} style={{
                ...
                background: status === "loading" ? "rgba(124,58,237,0.3)" : "linear-gradient(135deg, #7c3aed, #6366f1, #3b82f6)",
                color: "#fff", fontWeight: 700, fontSize: "16px",
                boxShadow: status === "loading" ? "none" : "0 4px 28px rgba(124,58,237,0.45)",
                transition: "all 0.25s",
              }}>
                {status === "loading" ? "Sending..." : "Free Audit Request भेजें →"}
              </button>

              <p style={{ textAlign: "center", fontSize: "12px", color: "rgba(228,220,255,0.25)", fontWeight: 500 }}>
                🔒 आपकी information 100% secure है।
              </p>
            </form>
          </div>
        </div>
